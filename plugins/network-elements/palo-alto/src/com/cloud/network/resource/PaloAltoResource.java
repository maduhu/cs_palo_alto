...
public class PaloAltoResource implements ServerResource {
    ...
    private boolean usageLogin() throws ExecutionException {
        ...
        xml = replaceXmlValue(xml, "username", _username);
        xml = replaceXmlValue(xml, "password", _password);
        return sendUsageRequestAndCheckResponse(PaloAltoCommand.LOGIN, xml);
    }
    ...
    private void openConfiguration() throws ExecutionException {
        ...
        if (!sendRequestAndCheckResponse(PaloAltoCommand.OPEN_CONFIGURATION, xml)) {
    ...
    private void closeConfiguration() {
        ...
        try {
            if (!sendRequestAndCheckResponse(PaloAltoCommand.CLOSE_CONFIGURATION, xml)) {
    ...
    private void implementGuestNetwork(GuestNetworkType type, Long publicVlanTag, String publicIp, long privateVlanTag, String privateGateway, String privateSubnet, long privateCidrNumber) throws ExecutionException {
        ...
        managePrivateInterface(PaloAltoCommand.ADD, !type.equals(GuestNetworkType.SOURCE_NAT), privateVlanTag, privateGateway);
        manageZoneInterface(PaloAltoCommand.ADD, privateVlanTag);
    ...
    private void shutdownGuestNetwork(GuestNetworkType type, long accountId, Long publicVlanTag, String sourceNatIpAddress, long privateVlanTag, String privateGateway, String privateSubnet, long privateCidrSize) throws ExecutionException {     
        ...
        managePrivateInterface(PaloAltoCommand.DELETE, false, privateVlanTag, privateGateway);  
    ...
    private Answer execute(SetFirewallRulesCommand cmd, int numRetries) {
        ...
        try {
            ...
            for (FirewallRuleTO rule : rules) {
                ...
                if (!rule.revoked()) {
                    manageFirewallFilter(PaloAltoCommand.ADD, term, _publicZoneInputFilterName);
                } else {
                    manageFirewallFilter(PaloAltoCommand.DELETE, term, _publicZoneInputFilterName);
                }
            }
    ...
    private void addStaticNatRule(Long publicVlanTag, String publicIp, String privateIp, List<FirewallRuleTO> rules) throws ExecutionException {
        manageProxyArp(PaloAltoCommand.ADD, publicVlanTag, publicIp);
        manageStaticNatRule(PaloAltoCommand.ADD, publicIp, privateIp);
        manageAddressBookEntry(PaloAltoCommand.ADD, _privateZone, privateIp, null);
    ...
    private void removeStaticNatRule(Long publicVlanTag, String publicIp, String privateIp) throws ExecutionException {     
        ...
        manageAddressBookEntry(PaloAltoCommand.DELETE, _privateZone, privateIp, null);     
    ...
    private void addDestinationNatRule(Protocol protocol, Long publicVlanTag, String publicIp, String privateIp, int srcPortStart, int srcPortEnd, int destPortStart, int destPortEnd) throws ExecutionException {
        ...
        for (int srcPort = srcPortStart; srcPort <= srcPortEnd; srcPort++) {
            ...
            manageDestinationNatPool(PaloAltoCommand.ADD, privateIp, destPort);      
            manageDestinationNatRule(PaloAltoCommand.ADD, publicIp, privateIp, srcPort, destPort); 
            offset += 1;
        }
    ...
    private void removeDestinationNatRule(Long publicVlanTag, String publicIp, String privateIp, int srcPort, int destPort) throws ExecutionException {               
        manageDestinationNatRule(PaloAltoCommand.DELETE, publicIp, privateIp, srcPort, destPort);
        manageDestinationNatPool(PaloAltoCommand.DELETE, privateIp, destPort);   
        manageProxyArp(PaloAltoCommand.DELETE, publicVlanTag, publicIp);    
    ...
    private boolean managePrivateInterface(PaloAltoCommand command, boolean addFilters, long vlanTag, String privateInterfaceIp) throws ExecutionException {
        String xml;

        switch (command) {

        case CHECK_IF_EXISTS:
            xml = PaloAltoXml.PRIVATE_INTERFACE_GETONE.getXml();
            xml = setDelete(xml, false);
        ...
        case ADD:
            if (managePrivateInterface(PaloAltoCommand.CHECK_IF_EXISTS, false, vlanTag, privateInterfaceIp)) {
                return true;
            }

            xml = addFilters ? PaloAltoXml.PRIVATE_INTERFACE_WITH_FILTERS_ADD.getXml() : PaloAltoXml.PRIVATE_INTERFACE_ADD.getXml();
            xml = replaceXmlValue(xml, "private-interface-name", _privateInterface);
            xml = replaceXmlValue(xml, "vlan-id", String.valueOf(vlanTag));
    ...
    private boolean manageProxyArp(PaloAltoCommand command, Long publicVlanTag, String publicIp) throws ExecutionException {
        ...
        case DELETE:
            ...
            if (manageProxyArp(PaloAltoCommand.CHECK_IF_IN_USE, publicVlanTag, publicIp)) {
                return true;
            }
    ...
    private boolean manageZoneInterface(PaloAltoCommand command, long vlanTag) throws ExecutionException {
        ...
        case CHECK_IF_EXISTS:
            xml = PaloAltoXml.ZONE_INTERFACE_GETONE.getXml();
            xml = setDelete(xml, false);
        ...
        case ADD:
            if (manageZoneInterface(PaloAltoCommand.CHECK_IF_EXISTS, vlanTag)) {
                return true;
            }

            xml = PaloAltoXml.ZONE_INTERFACE_ADD.getXml();
            xml = replaceXmlValue(xml, "private-zone-name", _privateZone);
            xml = replaceXmlValue(xml, "zone-interface-name", zoneInterfaceName);
    ...
    private boolean manageStaticNatRule(PaloAltoCommand command, String publicIp, String privateIp) throws ExecutionException {
        ...
        case ADD:
            if (manageStaticNatRule(PaloAltoCommand.CHECK_IF_EXISTS, publicIp, privateIp)) {
                return true;
            }
    ...
    private boolean manageDestinationNatPool(PaloAltoCommand command, String privateIp, long destPort) throws ExecutionException {
        ...
        case ADD:
            if (manageDestinationNatPool(PaloAltoCommand.CHECK_IF_EXISTS, privateIp, destPort)) {
                return true;
            }
        ...
        case DELETE:
            ...
            if (manageDestinationNatPool(PaloAltoCommand.CHECK_IF_IN_USE, privateIp, destPort)) {
                return true;
            }
    ...
    private boolean manageDestinationNatRule(PaloAltoCommand command, String publicIp, String privateIp, long srcPort, long destPort) throws ExecutionException {
        ...
        case ADD:
            if (manageDestinationNatRule(PaloAltoCommand.CHECK_IF_EXISTS, publicIp, privateIp, srcPort, destPort)) {
                return true;
            }
    ...
    private boolean manageSourceNatPool(PaloAltoCommand command, String publicIp) throws ExecutionException {
        String poolName = genSourceNatPoolName(publicIp);
        String xml;
        ...
        case CHECK_IF_EXISTS:
            xml = PaloAltoXml.SRC_NAT_POOL_GETONE.getXml();
            xml = setDelete(xml, false);
            xml = replaceXmlValue(xml, "pool-name", poolName);
            return sendRequestAndCheckResponse(command, xml, "name", poolName);
        ...
        case CHECK_IF_IN_USE:
            ...
            xml = PaloAltoXml.SRC_NAT_RULE_GETALL.getXml();
            return sendRequestAndCheckResponse(command, xml, "pool-name", poolName);

        case ADD:
            if (manageSourceNatPool(PaloAltoCommand.CHECK_IF_EXISTS, publicIp)) {
                return true;
            }

            xml = PaloAltoXml.SRC_NAT_POOL_ADD.getXml();
            xml = replaceXmlValue(xml, "pool-name", poolName);
        ...
        case DELETE:
            ...
            if (manageSourceNatPool(PaloAltoCommand.CHECK_IF_IN_USE, publicIp)) {
                return true;
            }
    ...
    private boolean manageSourceNatRule(PaloAltoCommand command, String publicIp, String privateSubnet) throws ExecutionException {
        ...
        case CHECK_IF_EXISTS:
            xml = PaloAltoXml.SRC_NAT_RULE_GETONE.getXml();
            xml = setDelete(xml, false);
        ...
        case ADD:
            if (manageSourceNatRule(PaloAltoCommand.CHECK_IF_EXISTS, publicIp, privateSubnet)) {
                return true;
            }
            ...
            xml = PaloAltoXml.SRC_NAT_RULE_ADD.getXml();
            xml = replaceXmlValue(xml, "rule-set", _privateZone);
            xml = replaceXmlValue(xml, "from-zone", _privateZone);
    ...
    private boolean manageAddressBookEntry(PaloAltoCommand command, String zone, String ip, String entryName) throws ExecutionException {
        ...
        case ADD:
            if (manageAddressBookEntry(PaloAltoCommand.CHECK_IF_EXISTS, zone, ip, entryName)) {
                return true;
            }
        ...
        case DELETE:
            ...
            if (manageAddressBookEntry(PaloAltoCommand.CHECK_IF_IN_USE, zone, ip, entryName)) {
                return true;
            }
    ...
    private boolean manageApplication(PaloAltoCommand command, Protocol protocol, int startPort, int endPort) throws ExecutionException {
        if (protocol.equals(Protocol.any)) {
            return true;
        }
        ...
        case ADD:
            if (manageApplication(PaloAltoCommand.CHECK_IF_EXISTS, protocol, startPort, endPort)) {
                return true;
            }
    ...
    private boolean manageSecurityPolicy(SecurityPolicyType type, PaloAltoCommand command, Long accountId, String username, String privateIp, List<String> applicationNames, String ipsecVpnName) throws ExecutionException {
        ...
        case ADD:
            if (!manageAddressBookEntry(PaloAltoCommand.CHECK_IF_EXISTS, toZone, privateIp, ipsecVpnName)) {
        ...
        case DELETE:
            if (!manageSecurityPolicy(type, PaloAltoCommand.CHECK_IF_EXISTS, null, null, privateIp, applicationNames, ipsecVpnName)) {
                return true;
            }

            if (manageSecurityPolicy(type, PaloAltoCommand.CHECK_IF_IN_USE, null, null, privateIp, applicationNames, ipsecVpnName)) {
                return true;
            }
    ...
    private boolean addSecurityPolicyAndApplications(SecurityPolicyType type, String privateIp, List<Object[]> applications) throws ExecutionException {
        ...
        for (Object[] application : applications) {         
            ...
            manageApplication(PaloAltoCommand.ADD, protocol, startPort, endPort);
        }

        // Add a new security policy
        manageSecurityPolicy(type, PaloAltoCommand.ADD, null, null, privateIp, applicationNames, null);

        return true;
    }

    private boolean removeSecurityPolicyAndApplications(SecurityPolicyType type, String privateIp) throws ExecutionException {
        if (!manageSecurityPolicy(type, PaloAltoCommand.CHECK_IF_EXISTS, null, null, privateIp, null, null)) {
            return true;
        }

        if (manageSecurityPolicy(type, PaloAltoCommand.CHECK_IF_IN_USE, null, null, privateIp, null, null)) {
            return true;
        }
        ...
        manageSecurityPolicy(type, PaloAltoCommand.DELETE, null, null, privateIp, null, null);
    ...
    private boolean sendRequestAndCheckResponse(PaloAltoCommand command, String xmlRequest, String... keyAndValue) throws ExecutionException {
        ...
        case CHECK_IF_EXISTS:
        case CHECK_IF_IN_USE:
            assert (keyAndValue != null && keyAndValue.length == 2) : "If the PaloAltoCommand is " + command + ", both a key and value must be specified.";
    ...
    private boolean sendUsageRequestAndCheckResponse(PaloAltoCommand command, String xmlRequest, String... keyAndValue) throws ExecutionException {                                                            
        boolean errorKeyAndValue = false;                                                                                                                                                                 
        String key;                                                                                                                                                                                       
        String value;                                                                                                                                                                                     
